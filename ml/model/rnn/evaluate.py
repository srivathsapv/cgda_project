...
def evaluate(test_data, model, dirpath_results, parameters, device, logger, exp_name):
    ...
    ax1.plot(range(len(trainacc)), trainacc,
             color='olivedrab', label='Train data')
    ax1.plot(range(len(testacc)), testacc,
             color='palevioletred', label='Test data')
    ...
    if exp_name == 'rnn_phylum':
        logger.info(
            'Since Phylum level has only 3 labels, creating scatter plot to visualize the encoded outputs along the three dimensions...')
        test_labels = []
        ...
        for i in range(len(label_weights)):
            if test_labels[i] == 0:
                c1 = ax.scatter(label_weights[i, 0], label_weights[i, 1],
                                label_weights[i, 2], color=colorlist[test_labels[i]], marker='*')
            elif test_labels[i] == 1:
                c2 = ax.scatter(label_weights[i, 0], label_weights[i, 1],
                                label_weights[i, 2], color=colorlist[test_labels[i]], marker='*')
            else:
                c3 = ax.scatter(label_weights[i, 0], label_weights[i, 1],
                                label_weights[i, 2], color=colorlist[test_labels[i]], marker='*')
        ax.legend((c1, c2, c3), ('Actinobacteria',
                                 'Firmicutes', 'Proteobacteria'), loc=0)
        ...
        logger.info(
            'Scatter plot of LSTM encoded components for Phylum Classifier can be found at '+scatterplot_filename)
